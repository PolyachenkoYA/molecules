...
def main():
    ...
    use_gpu = True
    if(use_gpu):
        N_arr = [512, 1024, 2048, 4096, 8192, 16384, 32768, 65536]
        N_arr = [512, 65536]
        N_arr = [65536]
        Np_arr = [8, 16, 32, 64, 128, 256]
        Np_arr = [8, 256]
        Np_arr = [512]
    else:
        N_arr = [16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536]
        N_arr = [512, 1024]
        Np_arr = [1, 2, 3, 4, 5, 8]
        Np_arr = [1, 2, 4]
    ...
    for Npi in Np_arr:
        ...
        for Ni in N_arr:
            new_name = 'N' + str(Ni) + '_p' + str(Npi)
            Nrep = max(int(round((5 if use_gpu else 2)*512 * (2048 / Ni)**2)), 4)
            endT = Nrep/512
            dumpDT = 2/endT
            if(use_gpu):
