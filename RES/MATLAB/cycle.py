...
def main():
    ...
    for ni in n:
	for Ti in T:
	    # sbatch --exclusive -N 1 --ntasks-per-node=1 --wrap="srun lmp_serial_noPNG -in in.msd_OK -var rho 0.02 -var t 3.0 -var filename n0.02_T3"
	    filename = 'n' + str(ni) + '_T' + str(Ti)
	    cmd = r'sbatch --exclusive -N 1 --ntasks-per-node=1 --wrap="srun lmp_serial_noPNG -in in.msd_OK -var rho ' + str(ni) + ' -var t ' + str(Ti) + ' -var filename ' + filename + r'"'
	    if(not run_it(cmd)):
		return
