...
params_names = ['Ntot', 'n', 'n_cr', 'r_cut', 'endT', 'dumpDT', 'dt', 'Tmp', 'dissipK', 'TmpStabEps', 'TmpStabGap', 'compMode', 'binOutF']
all_proc_flags = '-energy-maxwell-diff-cond-pics-percent-r_cm-r2-v_cm-r2_err'
...
def read_params(filename):
    ...
    params['Ntot'] = int(params['Ntot'])
    params['TmpStabGap'] = int(params['TmpStabGap'])
    params['binOutF'] = int(params['binOutF'])
...
def plot_error(fig_c, figs, x, y, y_th=[], x0=float('nan'), y0=float('nan'), x_lbl='time', y_lbl='error', tit = '', pic_path='', show_key=0, filter_rng=15, draw_linfit='y'):
    ...
    if(not math.isnan(filter_rng)):
        y_filt = signal.savgol_filter(y, int(round(filter_rng))*2 + 1, 2)
        plt.plot(x, y_filt, '--') 
